...
<div class="row">
    <div class="col-12">
        <div class="page-title-box d-flex align-items-center justify-content-between">
          ...
          <?php if(count($sTransfer_chooseAll)==0){ ?>
...
<!-- display: none; -->
  <?php if(count($sTransfer_chooseAll)==0){ ?>
      <div class="myBorder divAddTransferItem " style="" >
  <?php }else{ ?>
  ...
  <?php } ?>
         ...
         <form id="frm_save_to_transfer_list" action="{{ route('backend.transfer_branch_code.store') }}" method="POST" enctype="multipart/form-data" autocomplete="off">
            {{ csrf_field() }}
                ...
                <?php if(count($sTransfer_chooseAll)>0 && count($sTransfer_choose)==0){ ?>
                  <div class="row" style="" >
              ...
              rowCallback: function(nRow, aData, dataIndex){
                ...
                if(sU!=''&&sD!=''){
                  ...
                  if(aData['approve_status']==2){
                    ...
                    $('td:last-child', nRow).html(''
                      + '<a href="{{ route('backend.transfer_branch.index') }}/'+aData['id']+'/edit?role_group_id='+role_group_id+'&menu_id='+menu_id+'&list_id='+aData['id']+'" class="btn btn-sm btn-primary" style="'+sU+'" ><i class="bx bx-search font-size-18 align-middle"></i></a> '
                    ).addClass('input');
                    ...
                    $('td:last-child', nRow).html(''
                      + '<a href="{{ route('backend.transfer_branch.index') }}/'+aData['id']+'/edit?role_group_id='+role_group_id+'&menu_id='+menu_id+'&list_id='+aData['id']+'" class="btn btn-sm btn-primary" style="'+sU+'" ><i class="bx bx-search font-size-18 align-middle"></i></a> '
                      + '<a href="javascript: void(0);" data-id="'+aData['id']+'" class="btn btn-sm btn-danger cCancel " style="'+sD+'" title="ยกเลิก" ><i class="bx bx-x font-size-18 font-weight-bold align-middle"></i></a>'
                    ).addClass('input');
        ...
        "sDom": "<'row'<'col-sm-12'tr>><'row'<'col-sm-5'i><'col-sm-7'p>>",
           ...
           columns: [
                  ...
                  {data: 'branch_to',   title :'<center>โอนย้ายไปที่สาขา</center>', className: 'text-center',render: function(d) {
                    ...
                    if(aData['branch_id_fk_to']>0){

                      $('td:last-child', nRow).html(''
                        + '<input type="hidden" name="transfer_choose_id[]" value="'+aData['id']+'"> '
                        + '<a href="#" class="btn btn-sm btn-success btnEditToWarehouse " data-id="'+aData['id']+'" style="'+sU+'" ><i class="bx bx-edit font-size-16 align-middle"></i></a> '
                        + '<a href="javascript: void(0);" data-url="{{ route('backend.transfer_choose_branch.index') }}/'+aData['id']+'" class="btn btn-sm btn-danger cDelete" style="'+sD+'" ><i class="bx bx-trash font-size-16 align-middle"></i></a>'
                      ).addClass('input');

                    }else{

                      $('td:last-child', nRow).html(''
                        + '<a href="#" class="btn btn-sm btn-primary btnSetToWarehouse " data-id="'+aData['id']+'" style="'+sU+'" ><i class="bx bx-edit font-size-16 align-middle"></i></a> '
                        + '<a href="javascript: void(0);" data-url="{{ route('backend.transfer_choose_branch.index') }}/'+aData['id']+'" class="btn btn-sm btn-danger cDelete" style="'+sD+'" ><i class="bx bx-trash font-size-16 align-middle"></i></a>'
                      ).addClass('input');
                    }
