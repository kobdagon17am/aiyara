...
class Runpv_AiStockis extends Model
{
      ...
      if (empty($user) || empty($to_customer)) {
        ...
        if ($pv_total < 0) {
          ...
          if ($type == 1) { //ทำคุณสมบติ
            if ($update_to_customer->date_order_first == null || $update_to_customer->date_order_first == '' || $update_to_customer->date_order_first == '0000-00-00 00:00:00') {
            ...
            if ($update_to_customer->status_pv_mt == 'first') { //คุณไก่ขอเพิ่มทุกเงื่อนไข
            ...
            $resule =  RunPvController::Runpv($to_customer_user, $pv, $type, $transection_code);
            if ($to_customer_user->status_pv_mt == 'first') {//คุณไก่ขอเพิ่มทุกเงื่อนไข
              $start_month = date('Y-m');
              $mt_mount_new = strtotime("+2 Month", strtotime($start_month));
              //dd(date('Y-m-1',$mt_mount_new));
              $to_customer_user->pv_mt_active = date('Y-m-1',$mt_mount_new);
              $to_customer_user->status_pv_mt = 'not';
              $to_customer_user->date_mt_first = date('Y-m-d h:i:s');
              // $order_update->active_mt_date = date('Y-m-1',$mt_mount_new);
          }
  ...
  public static function run_pv_dropship($type, $pv, $to_customer_user, $username, $code_order = '', $order_id_fk = '')
  {
    try {
      DB::BeginTransaction();

      $user = DB::table('customers')
        ->select('id', 'drop_ship_bonus', 'user_name')
        ->where('user_name', '=', $username)
        ->first();


      $to_customer = DB::table('customers')
        ->select('id', 'drop_ship_bonus', 'user_name', 'pv_mt_active', 'pv_tv_active', 'status_pv_mt', 'pv_mt', 'pv_tv', 'pv')
        ->where('user_name', '=', $to_customer_user)
        ->first();
      ...
      if (empty($user) || empty($to_customer)) {
        ...
        $pv_total = $user->drop_ship_bonus - $pv;

        if ($pv_total < 0) {
          $resule = ['status' => 'fail', 'message' => 'ค่า Pv ไม่พอสำหรับการใช้งาน'];
        } else {
          $update_use->drop_ship_bonus = $pv_total;
