...
<div class="row">
    <div class="col-12">
        <div class="page-title-box d-flex align-items-center justify-content-between">
            <h4 class="mb-0 font-size-18"> สินค้า เบิก/ยืม </h4>
...
<!-- display: none; -->
   <?php if(!empty(@$sProducts_borrow_chooseAll)){ ?>
      <div class="myBorder divAddproducts_borrowItem " style="" >
  <?php }else{ ?>
      <div class="myBorder divAddproducts_borrowItem " >
  <?php } ?>


         <form id="frm_save_to_products_borrow_list" action="{{ route('backend.products_borrow_code.store') }}" method="POST" enctype="multipart/form-data" autocomplete="off">
            {{ csrf_field() }}
                ...
                <div class="row" >
                  <div class="col-8">
                       ...
                       <div class="form-group row">
                          <label for="borrow_cause" class="col-md-3 col-form-label"><i class="bx bx-play"></i>เหตุผลการเบิก :</label>
                          <div class="col-md-9">
                             <select id="borrow_cause_id_fk" name="borrow_cause_id_fk" class="form-control select2-templating " >
                              <option value="0">Select</option>}
                              @if(@$sBorrowCause)
                                @foreach(@$sBorrowCause AS $r)
                                <option value="{{$r->id}}" {{ (@$r->id==@$sRow->borrow_cause_id_fk)?'selected':'' }} >
                                  {{$r->txt_desc}}
                                </option>
                                @endforeach
                              @endif
                        ...
                        <div class="form-group row">
                          <label for="return_datetime" class="col-md-3 col-form-label"><i class="bx bx-play"></i>วัน เวลา กำหนดคืน :</label>
                          <div class="col-md-9">
                             <input class="form-control return_datetime"  autocomplete="off" placeholder="เวลา กำหนดส่งคืน" required=""  />
                ...
                <table id="data-table-to-borrow" class="table table-bordered dt-responsive" style="width: 100%;">
                  ...
                  <div class="row" style="" >
                    <div class="col-md-12 text-center divBtnSave "  >
                      ...
                      <i class="bx bx-save font-size-16 align-middle mr-1"></i> บันทึก สร้างใบยืม
                      </button>
                    </div>
                  </div>
                <?php } ?>
          ...
          <div class="myBorder">
                  ...
                  <div class="row">
                    <div class="col-12">
                      <div class="page-title-box d-flex align-items-center justify-content-between">
                        <h4 class="mb-0 font-size-18"><i class="bx bx-play"></i> รายการใบเบิก/ยืม </h4>
                      </div>
                    </div>
                  </div>
    ...
    <div class="modal-content">
      <div class="modal-header">
        ...
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
         ...
         <form action="{{ route('backend.products_borrow.store') }}" method="POST" enctype="multipart/form-data" autocomplete="off">
            <input type="hidden" name="save_select_to_products_borrow" value="1" >
            <input type="hidden" id="branch_id_select_to_products_borrow" name="branch_id_select_to_products_borrow" >
            {{ csrf_field() }}
...
<div class="modal fade" id="setToWarehouseModal" tabindex="-1" role="dialog" aria-labelledby="setToWarehouseModalTitle" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered modal-lg " role="document" style="max-width: 1000px !important;">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="setToWarehouseModalTitle"><b><i class="bx bx-play"></i>เลือกคลังที่เบิก/ยืม</b></h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
...
<div class="modal fade" id="setToWarehouseModalEdit" tabindex="-1" role="dialog" aria-labelledby="setToWarehouseModalEditTitle" aria-hidden="true">
  <div class="modal-dialog modal-dialog-centered modal-lg " role="document" style="max-width: 1000px !important;">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="setToWarehouseModalEditTitle"><b><i class="bx bx-play"></i>เลือกคลังที่เบิก/ยืม</b></h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
...
$(document).ready(function() {
      ...
      $(function() {
          ...
          oTable = $('#data-table-products_borrow-list').DataTable({
          "sDom": "<'row'<'col-sm-12'tr>><'row'<'col-sm-5'i><'col-sm-7'p>>",
              ...
              columns: [
                  {data: 'borrow_number', title :'รหัสใบยืม', className: 'text-center w80'},
              ...
              rowCallback: function(nRow, aData, dataIndex){
                ...
                // }else{ 
                  ...
                  if(aData['approve_status']!=0){
                    ...
                    // $('td:eq(6)', nRow).html( '' );
                    $('td:last-child', nRow).html(''
                      + '<a href="{{ route('backend.products_borrow.index') }}/'+aData['id']+'/edit?role_group_id='+role_group_id+'&menu_id='+menu_id+'&list_id='+aData['id']+'" class="btn btn-sm btn-primary" style="font-size:16px;padding-top:0px !important;padding-bottom:0px !important;" ><i class="mdi mdi-eye-outline align-middle" ></i></a> '
    ...
    $(document).ready(function() {
        $(document).on('click', '.btnSearch', function(event) {
          ...
          if(branch_id_fk==''){
            ...
            return false;
          }else if(borrow_cause_id_fk=='0'){
            $("#borrow_cause_id_fk").select2('open');
          }else if(return_datetime==''){
              // $(".return_datetime").focus();
              // $(".return_datetime").click();
              setTimeout('$(".return_datetime").focus();', 320);
              return false;
          }else if(product_id==''){
  					...
  					$(function() {
                        ...
                        {data: 'amt',   title :'<center>จำนวนที่มีในคลัง</center>', className: 'text-center',render: function(d) {
                           return '<center>'+(d)+'<input type="hidden" class="amt_in_warehouse" value="'+(d)+'" > ' ;
  					            ...
  					            {data: 'id',   title :'<center>จำนวนยืม</center>', className: 'text-center',render: function(d) {
  					               return '<center><input class="form-control amt_to_products_borrow in-tx  " type="number"  name="amt_products_borrow[]" style="background-color:#e6ffff;border: 2px inset #EBE9ED;width:60%;text-align:center;" ><input type="hidden" name="id[]" value="'+(d)+'" >' ;
  					            }},
    ...
    var oTable;
    $(function() {
        oTable = $('#data-table-to-borrow').DataTable({
        "sDom": "<'row'<'col-sm-12'tr>><'row'<'col-sm-5'i><'col-sm-7'p>>",
            processing: true,
            serverSide: true,
            ordering: false,
            ...
            // paging: false,
            ajax: {
              url: '{{ route('backend.products_borrow_choose.datatable') }}',
              data: function ( d ) {
                d.Where={};
    ...
    $(document).ready(function() {
       $(document).on('change', '.amt_to_products_borrow', function(event) {
           event.preventDefault();
           var amt_to_products_borrow = $(this).val();
           var amt_in_warehouse = $(this).closest("tr").find(".amt_in_warehouse").val();
           // alert(amt_to_products_borrow+":"+amt_in_warehouse);
           var a = parseInt(amt_to_products_borrow);
           var b = parseInt(amt_in_warehouse);
           if(a>b){
              alert("!!! จำนวนไม่ถูกต้อง จำนวนที่ยืมควรมีค่าน้อยกว่าหรือเท่ากับจำนวนที่มีในคลัง");
              $(this).val("");
              return false;
           }
       });
    ...
    $(document).ready(function() {
      ...
      // $("#exampleModalCenter").modal({ show : true });
       ...
       $(window).keydown(function(event){
        ...
        $(document).on('click', '.btnSave', function(event) {
           ...
           $("#spinner_frame").show();
            ...
            setTimeout(function(){
              ...
              // $("#spinner_frame").hide();
              // location.reload();
              $("#frm_save_to_products_borrow_list").submit();
            }, 1500);
        ...
        $(document).on('click', '.btnAddproducts_borrowItem', function(event) {
            // event.preventDefault();
            $("#spinner_frame").show();
            setTimeout(function(){
              // $(".divAddproducts_borrowItem").show();
              $(".divAddproducts_borrowItem").toggle();
              $("#spinner_frame").hide();
            }, 500);
            // $(".divAddproducts_borrowItem").toggle();
        });
        ...
        //     var d = $(this).val();
        //     var return_date = d.split("/").reverse().join("-");
        //     $("#return_date").val(return_date);
        //     localStorage.setItem('return_date2', return_date);
        // });
        // if(localStorage.getItem('return_date')){
        //   $('.return_date').val(localStorage.getItem('return_date'));
        //   $('#return_date').val(localStorage.getItem('return_date2'));
        // }


        $('.return_datetime').change(function() {
            localStorage.setItem('return_datetime', this.value);
        });
        if(localStorage.getItem('return_datetime')){
          $('.return_datetime').val(localStorage.getItem('return_datetime'));
          $('#return_datetime').val(localStorage.getItem('return_datetime'));
        }
        ...
        $(document).on('change', '#borrow_cause_id_fk', function(event) {
            event.preventDefault();
            var id = $(this).val();
            localStorage.setItem('borrow_cause_id_fk', id);
        });

        if(localStorage.getItem('borrow_cause_id_fk')){
            $('#borrow_cause_id_fk').val(localStorage.getItem('borrow_cause_id_fk')).select2();
        }
