      ...
      // dd($rs->all());
        ...
        if($rs->select_status != 3){
          if($rs->btn_type=='display'){
           ...
           $pdf = PDF::loadView('backend.total_thai_cambodia.print_day_total_display',[
             'report_type' => $report_type,
             'startDate' => $startDate,
             'endDate' => $endDate,
             'startDate2' => $startDate2,
         ...
         //    return $pdf->download('day_total.pdf'); // โหลดทันที
          ...
          if($rs->btn_type=='display'){
            return view('backend.total_thai_cambodia.print_day_total_display_transfer_over',[
             'report_type' => $report_type,
             'startDate' => $startDate,
             'endDate' => $endDate,
             'startDate2' => $startDate2,
           ...
           $pdf = PDF::loadView('backend.total_thai_cambodia.print_day_total_display_transfer_over',[
             'report_type' => $report_type,
             'startDate' => $startDate,
             'endDate' => $endDate,
             'startDate2' => $startDate2,
 ...
 public function commission_transfer_af_pdf(Request $rs)
  ...
  //    return $pdf->download('day_total.pdf'); // โหลดทันที
   ...
   public function createPDFRequisitionDetailRealRemain($packing_code_id)
   {
      ...
      $data = [$packing_code_id];
      // $pdf = PDF::loadView('backend.delivery_packing.print_receipt',compact('data'));
      $pdf = PDF::loadView('backend.pick_warehouse.print_requisition_detail_real_remain',compact('data'));
      // return $pdf->download('cover_sheet.pdf'); // โหลดทันที
      return $pdf->stream('receipt_sheet.pdf'); // เปิดไฟลฺ์
     ...
     /*
        ...
        */
        ...
        /*
            ...
            2   กรุงเทพฯ และปริมณฑล / Metropolitan area
            3   ต่างจังหวัด / Upcountry
            4   ส่งแบบพิเศษ / Premium  > $request->delivery_location_05 delivery_location
        */
        if(!empty($request->province_id)){
            $province_id = $request->province_id;
        }else{
              ...
              // shipping_cost
                DB::select(" UPDATE db_orders SET shipping_price=0,shipping_free=0  WHERE id=$frontstore_id ");
                 ...
                 if($delivery_location==0 || $delivery_location==4){ //รับสินค้าด้วยตัวเอง / จัดส่งพร้อมบิลอื่น
                    ...
                    // return $request->branch_id_fk;
                    DB::select(" UPDATE db_orders SET delivery_location=$delivery_location WHERE id=$frontstore_id ");
                    return 0 ;
                    // dd();
                }else{
   ...
   public function ajaxCalPriceFrontstore04(Request $request)
    {
        ...
        $sum_price = ($sum_price+$shipping_price) ;
            ...
            @$request->aicash_price  = @$request->aicash_price!= 0 ? @$request->aicash_price : 0 ;
            @$request->aicash_remain = @$request->aicash_remain!= 0 ? @$request->aicash_remain : 0 ;
                      ...
                      if($_choose->count() == 0){
                            ...
                            foreach($db_pay_requisition_002s as $data_002){
                                    DB::table('db_stocks_return')->insert([
                                        ...
                                        'business_location_id_fk' => $data_002->business_location_id_fk,
                                        'branch_id_fk' => $data_002->branch_id_fk,
                                        'product_id_fk' => $data_002->product_id_fk,
                                        'lot_number' => $data_002->lot_number,
                                        'lot_expired_date' => $data_002->lot_expired_date,
                                        'amt' => $data_002->amt_get,
                                        'product_unit_id_fk' => $data_002->product_unit_id_fk,
                                        'warehouse_id_fk' => $data_002->warehouse_id_fk,
                                        'zone_id_fk' => $data_002->zone_id_fk,
                                        'shelf_id_fk' => $data_002->shelf_id_fk,
                                        'shelf_floor' => $data_002->shelf_floor,
                                        'pick_pack_requisition_code_id_fk' => $data_002->pick_pack_requisition_code_id_fk,
                                        'created_at' => $data_002->created_at,
                                        'updated_at' => now(),
                                        'status_cancel' => 1,
    ...
    public function ajaxDelDelivery(Request $request)
    {
      ...
      if($request->ajax()){
          ...
          foreach ($DP as $key => $value) {
              DB::update(" UPDATE db_delivery SET status_pack='0',`packing_code`='0',`status_to_wh`='0',`status_to_wh_by`='0', `packing_code_desc`=NULL WHERE id = ".$value->delivery_id_fk."  ");
          }
