        ...
        @if (session('alert'))
            ...
            <script type="text/javascript">
                $(function() {
                    toastr["{{ strtolower(session('alert.status')) }}"]($('#toastr').html());
                });
            </script>
        @endif
        ...
        <script type="text/javascript">
            $(document).on('click', '.click_link', function(event) {
                ...
                // alert("xxxxxx");
                var role_group_id =
                    "{{ !empty(\Auth::user()->role_group_id_fk) ? \Auth::user()->role_group_id_fk : 0 }}";
            ...
            $(document).ready(function() {
                ...
                if (sPermission != 1) {
                    ...
                    (can_approve == 1) ? ($(".div_approve_transfer_branch_get").show()) : ($(
                        ".div_approve_transfer_branch_get").remove());
                    (can_approve == 1) ? ($(".div_confirm_transfer_slip").show()) : ($(".div_confirm_transfer_slip")
                        .remove());
                    (can_approve == 1) ? ($(".div_confirm_transfer_branch").show()) : ($(".div_confirm_transfer_branch")
                        .remove());
                    (can_approve == 1) ? ($(".div_confirm_transfer_warehouses").show()) : ($(
                        ".div_confirm_transfer_warehouses").remove());
                    (can_approve == 1) ? ($(".div_confirm_general_receive").show()) : ($(".div_confirm_general_receive")
                        .remove());
                    (can_approve == 1) ? ($(".div_confirm_general_takeout").show()) : ($(".div_confirm_general_takeout")
                        .remove());
                    (can_approve == 1) ? ($(".div_confirm_products_borrow").show()) : ($(".div_confirm_products_borrow")
                        .remove());
                    (can_approve == 1) ? ($(".div_confirm_transfer_warehouses").show()) : ($(
                        ".div_confirm_transfer_warehouses").remove());
