   ...
   public function form($id=NULL)
    ...
    public function Datatable(Request $request){
      ...
      if(!empty($request->stock_id) && !empty($request->start_date) && !empty($request->end_date)){
          ...
          // $Stock_movement = \App\Models\Backend\Stock_movement::where('stock_id_fk',$request->stock_id)->get();

            $Stock = \App\Models\Backend\Check_stock::where('id',$request->stock_id)->get();


// รายการก่อน start_date เพื่อหายอดยกมา
            $Stock_movement_remain = \App\Models\Backend\Stock_movement::
            ...
            $txt = "ยอดคงเหลือยกมา";
            $amt_balance_stock = @$Stock_movement_remain[0]->sum?$Stock_movement_remain[0]->sum:0;
  ...
  // รายการตามช่วงวันที่ระบุ start_date to end_date
            ...
            // ถ้าวันที่ $request->start_date > ปัจจุบัน ให้เอายอด คงเหลือปัจจุบัน เลย
