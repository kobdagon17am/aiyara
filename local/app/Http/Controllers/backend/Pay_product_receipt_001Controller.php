...
// เก็บลงตารางจริง
          ...
          if($lastInsertId){
               ...
               // เก็บรายการสินค้าที่จ่าย 
                ...
                DB::select(" INSERT IGNORE INTO  db_pay_product_receipt_002_pay_history (invoice_code,pay_date,pay_user,amt_need,amt_get,amt_remain) select invoice_code,now(),".\Auth::user()->id.",amt_need,amt_get,amt_remain FROM  $temp_ppr_004 ");
              ...
              // ตัด Stock 
              // $db_select = DB::select(" 
              //   SELECT * FROM  temp_db_pay_product_receipt_006 WHERE invoice_code='$invoice_code'
              //    ");
    ...
    public function Datatable001(Request $req){
       ...
       $w08 = "";
        ...
        if(isset($req->status_sent) && $req->status_sent!=0 ){
           ...
           $w08 = " AND date(db_pay_product_receipt_001.pay_date)=CURDATE() ";
        }
...
// เก็บประวัติการเลือก
    public function Datatable007(Request $req){
      ...
      $sTable = DB::select(" SELECT * FROM db_pay_product_receipt_001  WHERE 1 ".$w001."  ");
      $sQuery = \DataTables::of($sTable);
      return $sQuery
      ->addColumn('column_001', function($row) {
          return "<b>".@$row->invoice_code."</b>";
    ...
    public function Datatable008(Request $req){
      ...
      $sTable = DB::select(" SELECT * FROM db_pay_product_receipt_001  WHERE  db_pay_product_receipt_001.invoice_code='".$req->txtSearch."'  ");
      $sQuery = \DataTables::of($sTable);
      return $sQuery
      ->addColumn('column_001', function($row) { 
            ...
            $rs = DB::select(" SELECT
                  ...
                  Left Join ck_users_admin ON db_pay_product_receipt_002_pay_history.pay_user = ck_users_admin.id  WHERE invoice_code='".$row->invoice_code."' group by time_pay order by time_pay asc ");
      ...
      ->addColumn('column_002', function($row) {
          ...
          $Products = DB::select("
             SELECT db_pay_product_receipt_002.*,sum(amt_get) as amt_get_sum from db_pay_product_receipt_002 
             where db_pay_product_receipt_002.invoice_code='".$row->invoice_code."' group by product_id_fk order by time_pay asc 
          ");
          ...
          foreach ($Products as $key => $v) {
            ...
            $css_font = $v->amt_remain>0?"color:red;font-weight:bold;":"";
                    ...
                    <div class="divTableCell" style="text-align:center;"> '.$v->amt_get_sum.'</div>
                    <div class="divTableCell" style="text-align:center;'.$css_font.'"> '.$v->amt_remain.'</div>
                    <div class="divTableCell" style="text-align:center;width:450px;padding-bottom:15px !important;"> 
                    ';
                      ...
                      foreach ($WH AS $v_wh) {
                          ...
                          $pn .=     
                              ...
                              <div class="divTableCell" style="width:100px;text-align:center;color:blue;font-weight:bold;">'.($v_wh->amt_get>0?$v_wh->amt_get:'').'</div>
                              </div>
                              ';
