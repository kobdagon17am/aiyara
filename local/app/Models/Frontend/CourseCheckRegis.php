...
class CourseCheckRegis extends Model
{
    ...
    public static function check_register_all($course_id,$customer_username){
      $data_ce = DB::table('course_event')
      ->where('id',$course_id)
      ->first();
      ...
      if($qualification_id < $data_ce->reward_qualify_purchased and !empty($data_ce->reward_qualify_purchased)){
          $resule[] = ['status'=>'fail','message'=>'ไม่ผ่านคุณวุฒิ Reward ขั้นต่ำที่ซื้อได้','code'=>'e003'];
      }

      if($data_ce->keep_personal_quality == 1 and !empty($data_ce->keep_personal_quality) and $mt_active['status'] == 'fail'){
          ///ต้องรักษาคุณสมบัตรรายเดือน
       $resule[] = ['status'=>'fail','message'=>$mt_active['message'],'code'=>'e004'];
      }

    if($data_ce->keep_personal_quality == 1 and !empty($data_ce->keep_personal_quality) and $mt_active['type']=='N'){

      $resule[] = ['status'=>'fail','message'=>'ต้องรักษาคุณสมบัติรายเดือน','code'=>'e005'];
    }

    if($data_ce->keep_personal_quality == 2 and !empty($data_ce->keep_personal_quality) and $mt_active['status'] == 'fail'){
              ///ไม่ต้องรักษาคุณสมบัตรรายเดือน
      $resule[] = ['status'=>'fail','message'=>$mt_active['message'],'code'=>'e006'];
    }

    if($data_ce->keep_personal_quality == 2 and !empty($data_ce->keep_personal_quality) and $mt_active['type']=='Y'){
      $resule[] = ['status'=>'fail','message'=>'ต้องไม่รักษาคุณสมบัติรายเดือน','code'=>'e007'];
    }

    if($data_ce->maintain_travel_feature == 1 and !empty($data_ce->maintain_travel_feature) and $mt_active['status'] == 'fail'){
              ///ต้องรักษาคุณสมบัตรรายเดือน
      $resule[] = ['status'=>'fail','message'=>$tv_active['message'],'code'=>'e008'];
    }

    if($data_ce->maintain_travel_feature == 1 and !empty($data_ce->maintain_travel_feature) and $tv_active['type']=='N'){

      $resule[] = ['status'=>'fail','message'=>'ต้องรักษาคุณสมบัติท่องเที่ยว','code'=>'e009'];
    }

    if($data_ce->maintain_travel_feature == 2 and !empty($data_ce->maintain_travel_feature) and $tv_active['status'] == 'fail'){
                          ///ไม่ต้องรักษาคุณสมบัตรรายเดือน
      $resule[] = ['status'=>'fail','message'=>$tv_active['message'],'code'=>'e010'];
    }

    if($data_ce->maintain_travel_feature == 2 and !empty($data_ce->maintain_travel_feature) and $tv_active['type']=='Y'){
      $resule[] = ['status'=>'fail','message'=>'ต้องไม่รักษาคุณสมบัติท่องเที่ยว','code'=>'e011'];
    }

    if($data_ce->aistockist == 1 and !empty($data_ce->aistockist) and ($aistockist_status == 0 || $aistockist_status == null)){//เป็น aistockist

      $resule[] = ['status'=>'fail','message'=>'ต้องเป็น  Ai-Stockist','code'=>'e012'];
    }

    if($data_ce->aistockist == 2 and !empty($data_ce->aistockist) and $aistockist_status == 1){//ต้องไม่เป็น aistockist
