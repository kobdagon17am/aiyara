            ...
            <div class="card-body">
                <div class="row" style="background-color: #ccffff;"  >
                  <div class="col-8" style="display: flex;flex-direction: row;" >
                    ...
                    <h4 class="mb-0 font-size-18" style="padding: 1%;"> รายการจ่ายรออนุมัติ </h4>
                  ...
                  <div class="col-4 text-right" style="{{@$sC}}">
                    <a class="btn btn-info btn-sm mt-1 " href="{{ route('backend.products_out.create') }}?role_group_id={{$role_group_id}}&menu_id={{$menu_id}}">
...
$(function() {
    ...
    "sDom": "<'row'<'col-sm-12'tr>><'row'<'col-sm-5'i><'col-sm-7'p>>",
        ...
        iDisplayLength: 25,
        ajax: {
          url: '{{ route('backend.products_out.datatable') }}',
          data: function ( d ) {
            d.Where={};
        ...
        columns: [
            ...
            {data: 'reason', title :'<center>สาเหตุหลัก </center>', className: 'text-center'},
            {data: 'note', title :'<center>หมายเหตุ </center>', className: 'text-center'},
            {data: 'return_date', title :'<center>วันที่จ่ายออก </center>', className: 'text-center'},
            

            {data: 'receptionist_name', title :'<center>พนักงานที่บันทึก </center>', className: 'text-center'},
        ...
        rowCallback: function(nRow, aData, dataIndex){
          if(sU!=''&&sD!=''){
              ...
              $('td:last-child', nRow).html(''
                + '<a href="{{ route('backend.products_out.index') }}/'+aData['id']+'/edit?role_group_id='+role_group_id+'&menu_id='+menu_id+'" class="btn btn-sm btn-primary" style="'+sU+'" ><i class="bx bx-edit font-size-16 align-middle"></i></a> '
                + '<a href="javascript: void(0);" data-url="{{ route('backend.products_out.index') }}/'+aData['id']+'" class="btn btn-sm btn-danger cDelete" style="'+sD+'" ><i class="bx bx-trash font-size-16 align-middle"></i></a>'
              ).addClass('input');
