...
class Products_outController extends Controller
{

    public function index(Request $request)
    {

      return view('backend.products_out.index');
 ...
 public function create()
    {
      ...
      return View('backend.products_out.form')->with(
        array(
    ...
    {
       $sRow = \App\Models\Backend\Products_out::find($id);
       $Province = DB::select(" select * from dataset_provinces ");
   ...
   public function form($id=NULL)
    {
      ...
      try {
          if( $id ){
            ...
            $sRow = new \App\Models\Backend\Products_out;
          }
          ...
          \DB::commit();
           ...
           return redirect()->to(url("backend/Products_out/".$sRow->id."/edit?role_group_id=".request('role_group_id')."&menu_id=".request('menu_id')));
           

      } catch (\Exception $e) {
        echo $e->getMessage();
        \DB::rollback();
        return redirect()->action('backend\Products_outController@index')->with(['alert'=>\App\Models\Alert::e($e)]);
      }
    }
    ...
    public function Datatable(){
      $sTable = \App\Models\Backend\Products_out::search()->orderBy('id', 'asc');
