...
class Po_approveController extends Controller
{
  ...
  public function edit($id)
  {
    ...
    $p_bill = "บิล : " . $sRow->code_order . '<input type="hidden" class="order_id_approve" name="order_id_approve[]" value="'.$sRow->id.'"> <br> ยอดชำระ : <label style="color:red;">' . $sRow->transfer_price . ' </label><br>';

    // วุฒิเพิ่มวนเช็คว่ามีบิลไหนจ่ายพร้อมบิลนี้ไหม
    $other_bill = DB::table('db_orders')
      ->select('id','code_order','transfer_price')
    ...
    foreach ($other_bill as $ot) {
      ...
      $p_bill .= "บิล : " . $ot->code_order . '<input type="hidden" class="order_id_approve" name="order_id_approve[]" value="'.$ot->id.'"> <br> ยอดชำระ : <label style="color:red;"> ' . $ot->transfer_price . ' </label><br>';
    }
