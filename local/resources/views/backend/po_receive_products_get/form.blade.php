...
<div class="row">
    <div class="col-10">
        ...
        <div class="myBorder">
              ...
              @else
              <form action="{{ route('backend.po_receive_products_get.update', @$sRow->id ) }}" method="POST" enctype="multipart/form-data" autocomplete="off">
                <input name="_method" type="hidden" value="PUT">
                <input type="hidden" name="po_supplier_products_id_fk" value="{{@$sRow->id}}">
              @endif
                {{ csrf_field() }}
...
<a class="btn btn-secondary btn-sm waves-effect" href="{{ route('backend.po_receive_products.index') }}/{{@$Po_supplier_products->id}}/edit" }}">
...
@section('script')
  ...
  <script>
            ...
            var po_receive_products_get_id_fk = "{{@$sRow->id?@$sRow->id:0}}"; console.log(po_receive_products_get_id_fk);
            var oTable;
            ...
            $(function() {
                ...
                "sDom": "<'row'<'col-sm-12'tr>><'row'<'col-sm-5'i><'col-sm-7'p>>",
                    ...
                    iDisplayLength: 5,
                    ajax: {
                            url: '{{ route('backend.po_receive_products_get.datatable') }}',
                            data: function ( d ) {
                                    d.Where={};
                                    d.Where['po_receive_products_get_id_fk'] = po_receive_products_get_id_fk ;
                                    oData = d;
                                  },
                              method: 'POST',
                            },
