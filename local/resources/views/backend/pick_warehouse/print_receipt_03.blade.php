...
db_pick_pack_requisition_code.pick_pack_packing_code
FROM
...
foreach($delivery as $deli){
  $x = explode(",",$deli->receipts);
  for ($i=0; $i < count($x); $i++) {
    array_push($arr01,"'".$x[$i]."'");
  }
}
$arr01 = implode(",",$arr01);
 // print_r($arr01);
...
$value = DB::select("
                    ...
                    customers_detail.amphures_id_fk as district,
                    customers_detail.district_id_fk as district_sub,
                    customers_detail.road,
                    customers_detail.province_id_fk as province,
      ...
      <table style="border-collapse: collapse;" >
        <tr>
          ...
          <th style="text-align: right;">
            (<?php echo sprintf("%04d",$front_db->id); ?>)<br>
                      ...
                      if(@$sRow->delivery_location==0){
                        ...
                        if(@$sRow->delivery_location==1){
                          ...
                          $addr = DB::select(" SELECT
                                      ...
                                      customers_address_card.card_amphures_id_fk as card_district,
                                      customers_address_card.card_district_id_fk as card_district_sub,
                                      customers_address_card.card_road,
                                      customers_address_card.card_province_id_fk as card_province,
...
course_id_fk,action_date,action_user,approve_status,approver,approve_date,qr_code,created_at,updated_at,deleted_at from db_order_products_list WHERE frontstore_id_fk = ".$front_db->id." and add_from=2 GROUP BY promotion_id_fk,promotion_code
     ");
