...
class Total_thai_cambodiaController extends Controller
{
   ...
   // แบบไม่ sum
    ...
    public function Datatable_branch(Request $rs)
    {
        if ($rs->startDate) {
            $date = str_replace('/', '-', $rs->startDate);
            $s_date = date('Y-m-d', strtotime($date));
   ...
   // แบบไม่ sum
    ...
    public function Datatable_aicash(Request $rs)
    {
        if ($rs->startDate) {
            $date = str_replace('/', '-', $rs->startDate);
            $s_date = date('Y-m-d', strtotime($date));
            $startDate = " AND DATE(db_add_ai_cash.created_at) >= '" . $s_date . "' ";
        } else {
            $startDate = '';
        }
        ...
        if ($rs->endDate) {
            $date = str_replace('/', '-', $rs->endDate);
            $e_date = date('Y-m-d', strtotime($date));
            $endDate = " AND DATE(db_add_ai_cash.created_at) <= '" . $e_date . "' ";
        } else {
            $endDate = '';
        }
   ...
   // แบบไม่ sum
        if($report_type == 'day'){
            ...
            db_add_ai_cash.gift_voucher_price,
